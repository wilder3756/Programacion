...
void *enviarMensaje(void* args){
  int len;
  buffer[0].mtype = 10; //el chat 2 envia mensajes cuyo tipo es 10
  while(1) {
...
void* recibirMensaje(void *args){
  while(1){
    //recuperar los mensajes del chat1 cuyo identificador de tipo es 20
    if (msgrcv(colaMensajes, &buffer[1], sizeof(buffer[1].mtext), 20, 0) == -1) {
      perror("msgrcv: ");
      exit(EXIT_FAILURE);
    }
